...
from photutils.detection import irafstarfind as isf
...
radius = 30 # radius from object center (fits coordinates) for backgroun estimation 
sigma = 3.0 # The number of standard deviations to use as the clipping limit. (background estimation)
...
class MyHandler(PatternMatchingEventHandler):
	...
	if solve_field(fits_coo):
	  solve_coo, solve_file_hdr, solve_file_data = open_solve_file(str(self.file_to_open).split(".")[0]+".new")
	  ...
	  if solve_file_hdr['OBJECT'] != pointing_object_name:
	    ...
	    fwhm_x, fwhm_y = star.get_fwhm(x, y, radius, solve_file_data, medv=None)
	    flux, aperture_area = star.phot(x, y, radius, solve_file_data, fwhm_x, fwhm_y)
	    signal_to_noise = star.snr(x, y, radius, solve_file_data, fwhm_x, fwhm_y, flux, aperture_area)
...
class Star:
  ...
  def snr(self, x, y, radius, data, fwhm_x, fwhm_y, flux, aperture_area):
    x0, y0, arr = self.cut_region(x, y, radius, data)
    mean, median, std = sigma_clipped_stats(arr, sigma=sigma)
    signal_to_noise = flux / math.sqrt(flux + aperture_area * math.pow(std,2))
...
def show_ds9(fits_coo, solve_coo):
  ...
  d.set('scale zscale')
  d.set('regions', 'fk5; line('+str(solve_coo.ra.deg)+','+str(solve_coo.dec.deg)+','+str(fits_coo.ra.deg)+','+str(fits_coo.dec.deg)+')')
  d.set('regions', 'fk5; circle('+str(fits_coo.ra.deg)+','+str(fits_coo.dec.deg)+',7")')
...
def solve_field(fits_coo):
  ...
  out = sub.Popen(solve_field_command, stdout=sub.PIPE, stderr=sub.PIPE).communicate()
...
def open_file(file_to_open):
  ...
  fits_coo = SkyCoord(hdr[ra_key]+" "+hdr[dec_key], 'icrs', unit=(u.hour, u.deg))
  return fits_coo
...
def pol_calc(flux, solve_file_hdr):
  ...
  if len(pol_flux_tab) == 4:
    u_st = (pol_flux_tab[1]-pol_flux_tab[0])/(pol_flux_tab[1]+pol_flux_tab[0])
    q_st = (pol_flux_tab[3]-pol_flux_tab[2])/(pol_flux_tab[3]+pol_flux_tab[2])
...
def plot(flux_tab, snr_tab, fwhm_tab, pol_tab, avg_pol):
  ...
  if pol_tab:
    ...
    ax4.axhline(y=avg_pol,xmin=0,xmax=len(pol_tab)*1.1,c="blue",linewidth=0.5,zorder=0)
  ax4.plot(pol_tab , 'ro')  
